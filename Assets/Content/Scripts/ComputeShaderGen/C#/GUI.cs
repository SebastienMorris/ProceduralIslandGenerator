...
public class GUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Create a simple white texture for drawing underlines
        underlineTexture = new Texture2D(1, 1);
        underlineTexture.SetPixel(0, 0, Color.white);
    ...
    private void OnGUI()
    {
        ...
        if (headerLabel == null)
        {
            headerLabel = new GUIStyle(UnityEngine.GUI.skin.label);
    ...
    private int DraggableIntField(string label, int value, string fieldId, float sensitivity = 1f)
    {
        ...
        Rect labelRect =
            GUILayoutUtility.GetRect(new GUIContent(label), UnityEngine.GUI.skin.label, GUILayout.Width(40));
        ...
        if (isHovering)
        {
            ...
            UnityEngine.GUI.DrawTexture(underlineRect, underlineTexture, ScaleMode.StretchToFill, true, 0,
                new Color(0.3f, 0.6f, 1f, 0.8f), 0, 0);
        }
        ...
        {
            // Visual feedback for draggability - change cursor would go here if available
    ...
    private void DrawNoiseControls()
    {
        ...
        bool newApplyFallOff = GUILayout.Toggle(noiseSettings.applyFallOff, "");
        GUILayout.EndHorizontal();
        if (newApplyFallOff != noiseSettings.applyFallOff)
        {
            noiseSettings.applyFallOff = newApplyFallOff;
            chunkGenerator.NoiseSettings = noiseSettings;
        }
